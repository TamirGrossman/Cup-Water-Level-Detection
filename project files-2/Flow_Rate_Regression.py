...
Feature setup: "sound + duration".
  - All per-window acoustic features are used.
  - `duration_s` is KEPT as a feature.
  - The explicit `cup` id column is dropped from the features (the cup's
...
Run directly:  python Flow_Rate_Regression.py
"""
...
def load_and_build_target(
    ...
    # Features = everything EXCEPT the target-defining columns, but KEEP
    # duration_s. Drop the explicit cup id (sound already encodes the cup).
...
def run(csv_path: str, random_state: int = 42, run_cv: bool = True, make_plots: bool = True):
    ...
    print(f"Features: {X.shape[1]} (sound + duration) | Samples: {X.shape[0]}")
