...
from Data_Loader import load_data, preprocess_data, encode_target_to_classes, split_data, split_data_by_cup, scale_features, anova_f_scores
...
from Graph_results import plot_anova_f_scores, graph_CM
...
def train_classification_pipeline(
    ...
    model_XGB = build_xgb()
    model_Forest = buildRandomForest()
    ...
    if tune:
        ...
        model_Forest = buildRandomForest(**best_params_Forest, random_state=random_state)
...
def handle_data(
    ...
    return X_train_scaled, X_test_scaled, scaler, y_train, y_test, encoder, features
...
X_train_scaled, X_test_scaled, scaler, y_train, y_test, encoder, features = handle_data(
    'cup_dataset.csv', 
    target_column='Fill', 
...
results_XGB = model_pipeline(
    ...
    random_state= 42,
    use_feature_selection= False, max_features=50,
    run_cross_validation = True, cv = 4,
    tune = False, tune_grid= xgb_tune_grid, tune_cv= 4
    )
...
results_Forest = model_pipeline(
    ...
    random_state= 42,
    use_feature_selection= False, max_features=100,
    run_cross_validation = True, cv = 4,
